...
namespace HidLibrary
{
    public class HidDevice : IHidDevice
    {
        ...
        static extern bool ReadFile(IntPtr hFile, /*[Out] *//*byte[] lpBuffer*/IntPtr lpBuffer, uint nNumberOfBytesToRead, ref uint lpNumberOfBytesRead, IntPtr lpOverlapped);
            ...
            if (_deviceCapabilities.InputReportByteLength > 0)
            {
                ...
                if (_deviceReadMode == DeviceMode.Overlapped)
                {
                    ...
                    try
                    {
                        ...
                        //var success = ReadFile(Handle, buffer, (uint)buffer.Length, ref bytesRead, IntPtr.Zero);
                ...
                else
                {
                    try
                    {
                        ...
                        ReadFile(Handle, nonManagedBuffer, (uint)buffer.Length, ref bytesRead, IntPtr.Zero);
